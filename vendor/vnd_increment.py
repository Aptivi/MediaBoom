...
def vnd_increment(old_version, new_version, api_versions):
    ...
    installer_wxs = {
        f"{solution}/public/MediaBoom.Installers/MediaBoom.Installer/Package.wxs",
        f"{solution}/public/MediaBoom.Installers/MediaBoom.InstallerBundle/Bundle.wxs",
    }
    ...
    debian_changes_entry = f"""mediaboom-{version.new_api_package} ({version.new_api_version}-{version.new_version}-1) noble; urgency=medium
...
def process_arch_lines(arch_lines, version: VersionInfo):
    for num in range(0, len(arch_lines)):
        ...
        if is_pkgname:
            nitrocid_old_api_pkg = f'mediaboom-{version.old_api_package}'
            nitrocid_new_api_pkg = f'mediaboom-{version.new_api_package}'
            arch_lines[num] = \
...
def process_wxs_lines(lines, version: VersionInfo):
    for num in range(0, len(lines)):
        ...
        is_name = f'Name="MediaBoom {version.old_major}"' in lines[num]
        if is_ver:
            lines[num] = \
