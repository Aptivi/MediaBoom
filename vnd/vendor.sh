...
localize() {
    ...
    curl -L --output "$ROOTDIR/vnd/mpv-dev-x86_64-20260115-git-2007f55.7z" https://github.com/zhongfly/mpv-winbuild/releases/download/2026-01-15-2007f55/mpv-dev-x86_64-20260115-git-2007f55.7z
    curl -L --output "$ROOTDIR/vnd/mpv-dev-aarch64-20260115-git-2007f55.7z" https://github.com/zhongfly/mpv-winbuild/releases/download/2026-01-15-2007f55/mpv-dev-aarch64-20260115-git-2007f55.7z
...
prebuild() {
    ...
    if [ ! -f $ROOTDIR/vnd/mpv-dev-x86_64-20260115-git-2007f55.7z ]; then
        curl -L --output $ROOTDIR/vnd/mpv-dev-x86_64-20260115-git-2007f55.7z https://github.com/zhongfly/mpv-winbuild/releases/download/2026-01-15-2007f55/mpv-dev-x86_64-20260115-git-2007f55.7z
        checkvendorerror $?
    fi
    if [ ! -f $ROOTDIR/vnd/mpv-dev-aarch64-20260115-git-2007f55.7z ]; then
        curl -L --output $ROOTDIR/vnd/mpv-dev-aarch64-20260115-git-2007f55.7z https://github.com/zhongfly/mpv-winbuild/releases/download/2026-01-15-2007f55/mpv-dev-aarch64-20260115-git-2007f55.7z
        checkvendorerror $?
    fi

    # Install the DLL for AMD64
    cd $ROOTDIR/vnd && "$sevenzpath" x $ROOTDIR/vnd/mpv-dev-x86_64-20260115-git-2007f55.7z libmpv-2.dll && cd -
    checkvendorerror $?
    mkdir -p $ROOTDIR/public/MediaBoom.Native/runtimes/win-x64/native/
    checkvendorerror $?
    mv $ROOTDIR/vnd/libmpv-2.dll $ROOTDIR/public/MediaBoom.Native/runtimes/win-x64/native/
    checkvendorerror $?
    
    # Install the DLL for ARM64
    cd $ROOTDIR/vnd && "$sevenzpath" x $ROOTDIR/vnd/mpv-dev-aarch64-20260115-git-2007f55.7z libmpv-2.dll && cd -
    checkvendorerror $?
    mkdir -p $ROOTDIR/public/MediaBoom.Native/runtimes/win-arm64/native/
    checkvendorerror $?
    mv $ROOTDIR/vnd/libmpv-2.dll $ROOTDIR/public/MediaBoom.Native/runtimes/win-arm64/native/
    checkvendorerror $?
}
...
increment() {
    ...
    FILES=(
        ...
        "$ROOTDIR/public/MediaBoom.Installers/MediaBoom.Installer/Package.wxs"
        "$ROOTDIR/public/MediaBoom.Installers/MediaBoom.InstallerBundle/Bundle.wxs"
    ...
    for FILE in "${FILES[@]}"; do
        ...
        sed -b -i "s/mediaboom-${APIVERSPLITOLD[2]}/mediaboom-${APIVERSPLITNEW[2]}/g" "$FILE"
        result=$?
        if [ $result -ne 0 ]; then
    ...
    sed -b -i "s/Name=\"MediaBoom $OLDMAJOR\"/Name=\"MediaBoom $NEWMAJOR\"/g" "$ROOTDIR/public/MediaBoom.Installers/MediaBoom.Installer/Package.wxs"
    ...
    sed -b -i "s/pkgname=mediaboom-${APIVERSPLITOLD[2]}/pkgname=mediaboom-${APIVERSPLITNEW[2]}/g" "$ROOTDIR"/PKGBUILD-VCS*
...
mediaboom-${APIVERSPLITNEW[2]} ($NEWAPIVER-$NEWVER-1) noble; urgency=medium
