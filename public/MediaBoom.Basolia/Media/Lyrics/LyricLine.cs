...
namespace MediaBoom.Basolia.Media.Lyrics
{
    /// <summary>
    ...
    public class LyricLine
    {
        ...
        public void SeekLyric(BasoliaMedia? basolia)
        {
            ...
            lock (basolia.PositionLock)
            {
                ...
                if (!basolia.IsOpened())
                    throw new BasoliaException(LanguageTools.GetLocalized("MEDIABOOM_BASOLIA_PLAYBACK_EXCEPTION_FILENOTOPEN_SEEK"), MpvError.MPV_ERROR_GENERIC);

                // Get the length, convert it to frames, and seek
                long length = (long)LineSpan.TotalSeconds;
                basolia.SeekTo(length);
            }
        }
