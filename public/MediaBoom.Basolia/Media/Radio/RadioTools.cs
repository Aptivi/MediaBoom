...
namespace MediaBoom.Basolia.Media.Radio
{
    /// <summary>
    /// Radio tools
    /// </summary>
    public static class RadioTools
    {
        ...
        public static async Task<IRadioServer?> GetRadioInfoAsync(string radioUrl)
        {
            ...
            if (string.IsNullOrEmpty(radioUrl))
                throw new BasoliaMiscException(LanguageTools.GetLocalized("MEDIABOOM_BASOLIA_RADIO_EXCEPTION_NEEDSRADIOSTATIONPATH"));
            ...
            if (!reply.IsSuccessStatusCode)
                throw new BasoliaMiscException(LanguageTools.GetLocalized("MEDIABOOM_BASOLIA_FILE_EXCEPTION_NORADIOSTATION") + $" {(int)reply.StatusCode} ({reply.StatusCode}).");

            // Check for radio statio and get the MIME type
            if (!reply.Headers.Any((kvp) => kvp.Key.StartsWith("icy-")))
                throw new BasoliaMiscException(LanguageTools.GetLocalized("MEDIABOOM_BASOLIA_FILE_EXCEPTION_NOTARADIOSTATION"));
            ...
            if (reply.Headers.Server.ToString().ToLower().Contains("icecast"))
                ...
                throw new BasoliaMiscException(LanguageTools.GetLocalized("MEDIABOOM_BASOLIA_RADIO_EXCEPTION_SERVERTYPEFAILED"));
