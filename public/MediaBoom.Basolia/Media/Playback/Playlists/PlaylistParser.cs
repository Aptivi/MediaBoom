...
using MediaBoom.Basolia.Media.Playback.Playlists.Instances;
...
namespace MediaBoom.Basolia.Media.Playback.Playlists
{
    /// <summary>
    ...
    public static class PlaylistParser
    {
        ...
        public static Playlist ParsePlaylist(string playlistFile)
        {
            ...
            if (!FileIo.Exists(playlistFile))
                throw new BasoliaMiscException(LanguageTools.GetLocalized("MEDIABOOM_BASOLIA_PLAYBACK_PLAYLISTS_EXCEPTION_PLAYLISTFILENOTFOUND").FormatString(playlistFile));
            ...
            if (Path.HasExtension(playlistFile) && extension != PlaylistConstants.m3u && extension != PlaylistConstants.m3u8)
                throw new BasoliaMiscException(LanguageTools.GetLocalized("MEDIABOOM_BASOLIA_PLAYBACK_PLAYLISTS_EXCEPTION_PLAYLISTFILEEXTINVALID").FormatString(extension));
        ...
        public static Playlist ParsePlaylistFrom(string playlist, string fileParent = "")
        {
            ...
            if (string.IsNullOrEmpty(playlist))
                throw new BasoliaMiscException(LanguageTools.GetLocalized("MEDIABOOM_BASOLIA_PLAYBACK_PLAYLISTS_EXCEPTION_PLAYLISTFILEEMPTY"));
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                if (line.StartsWith("#") && i == 0)
                {
                    ...
                    if (header == PlaylistConstants.extendedHeader)
                        ...
                        throw new BasoliaMiscException(LanguageTools.GetLocalized("MEDIABOOM_BASOLIA_PLAYBACK_PLAYLISTS_EXCEPTION_NOTEXTHEADER") + $" [{header}]");
                    continue;
                }
                ...
                if (extended && line.StartsWith("#"))
                {
                    ...
                    if (declaration.StartsWith(PlaylistConstants.extendedInfo))
                    {
                        if (!declaration.Contains(':'))
                            throw new BasoliaMiscException(LanguageTools.GetLocalized("MEDIABOOM_BASOLIA_PLAYBACK_PLAYLISTS_EXCEPTION_EXTINFNEEDSARGS"));
                        trackInfoString = declaration.Substring(declaration.IndexOf(':') + 1);
                    }
                    continue;
                }

                // Check for music files or URLs
                ...
                else
                {
                    ...
                    if (FileIo.Exists(absolutePath))
                        ...
                        throw new BasoliaMiscException(LanguageTools.GetLocalized("MEDIABOOM_BASOLIA_PLAYBACK_PLAYLISTS_EXCEPTION_MUSICFILENOTFOUND").FormatString(absolutePath));
                }
                trackInfoString = "";
            }
            ...
            for (int i = 0; i < proposedTracks.Count; i++)
            {
                ...
                if (!string.IsNullOrWhiteSpace(trackInfo))
                {
                    if (!trackInfo.Contains(','))
                        throw new BasoliaMiscException(LanguageTools.GetLocalized("MEDIABOOM_BASOLIA_PLAYBACK_PLAYLISTS_EXCEPTION_TRACKINFONEEDSARGS"));
                    ...
                    if (!int.TryParse(lengthStr, out length))
                        throw new BasoliaMiscException(LanguageTools.GetLocalized("MEDIABOOM_BASOLIA_PLAYBACK_PLAYLISTS_EXCEPTION_TRACKINFOINVALIDSECS").FormatString(lengthStr));
